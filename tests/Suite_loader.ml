(** Bootstrap loader for the yaml-test-suite test files.
    The test suite stores its data in YAML format (src/*.yaml), but we cannot
    use YAMLx itself to parse them (it doesn't exist yet when we first run the
    tests).  This module is therefore a hand-written, line-based parser that
    understands the specific subset of YAML used by the test suite.
    ...
    Special visual characters in the [yaml:] field must be substituted:
      ...
      ∎  (U+220E)  →  strip the preceding and trailing newline
                       (indicates absence of final newline) *)
...
let substitute_visual (s : string) : string =
  ...
  while !i < n do
    ...
    (* ↵  U+21B5  E2 86 B5  (3 bytes) – empty line marker, just drop it *)
    else if remaining >= 3
       && Char.code s.[!i]     = 0xE2
    ...
    then
      advance_by 3 ""
    (* ⇔  U+21D4  E2 87 94  (3 bytes) – BOM *)
    else if remaining >= 3
       && Char.code s.[!i]     = 0xE2
       && Char.code s.[!i + 1] = 0x87
       && Char.code s.[!i + 2] = 0x94
    then
      advance_by 3 "\xEF\xBB\xBF"   (* UTF-8 encoded BOM *)
...
(** Strip a fixed [prefix_len] leading spaces from [s].
    If [s] has fewer than [prefix_len] spaces, return the stripped portion. *)
...
let parse_file (file_id : string) (content : string) : test_case list =
  ...
  let block_indent = 4 in  (* test files use 4 spaces for block scalar content *)
  ...
  let finish_block () =
    ...
    | Some field ->
      ...
      | _      -> ())  (* json, dump, emit: ignored for now *)
    );
    block_field := None;
    block_lines := []
  in
        ...
        | _ ->
          if starts_with "- " line then begin
            ...
            | key :: value_parts when String.trim key <> "" ->
              ...
              | "name" -> cur_name := value
              | "tags" ->
                cur_tags := String.split_on_char ' ' value
                            |> List.filter (fun s -> s <> "")
              | "fail" when value = "true" -> cur_fail := true
              | _ -> ())
            | _ -> ())
          end else if starts_with "  " line then begin
            let rest = String.sub line 2 (String.length line - 2) in
            let colon_idx =
              try Some (String.index rest ':') with Not_found -> None
            in
            (match colon_idx with
            | Some idx ->
              let key   = String.sub rest 0 idx |> String.trim in
              let after = String.sub rest (idx + 1) (String.length rest - idx - 1) in
...
(** Load all test cases from a single [.yaml] file.
    [path] must be the full file path; the file stem is used as the test id. *)
