(** YAML 1.2 parser.
    ...
    node              ::= ALIAS
                        | properties? (block_content | flow_content)
    ...
    The parser is implemented as a state machine.  [next_event] drives it
    one step at a time, returning one event per call. *)
...
(** The parser is a pushdown automaton.  [state] encodes the current
    production being processed and [states] is the return stack. *)
type parse_state =
  | Parse_stream_start
  ...
  | Parse_flow_sequence_need_separator  (** after an entry: requires ',' or ']' *)
...
(** Resolve a tag using the directive table.
    [(handle, suffix)] → full URI string. *)
let resolve_tag directives pos handle suffix =
  ...
  | _  ->
    ...
    | None        ->
      Types.parse_error pos "tag handle '%s' is not defined in this document" handle

(* ------------------------------------------------------------------ *)
(* Anchor / tag / alias collection                                       *)
(* ------------------------------------------------------------------ *)

(** Collect consecutive ANCHOR and TAG tokens (in any order) and return
    them together with the position of the first. *)
  ...
  while !collecting do
    ...
    | Some (Directive (name, value)) ->
      ...
      | "YAML" ->
        ...
        | [maj; min] ->
          (try
            p.directives.version <- Some (int_of_string maj, int_of_string min)
          with Failure _ -> ())
        | _ -> ())
      | handle when String.length handle >= 1 && handle.[0] = '!' ->
        (* TAG directive: name = handle, value = prefix *)
        p.directives.tags <-
          (handle, value) :: List.filter (fun (h,_) -> h <> handle) p.directives.tags
      | _ -> ())
    | _ -> collecting := false
  done;
  !had_directives
...
(** Produce one event from the current state.  Updates [p.state] to the
    next state. *)
let rec produce p =
  match p.state with
  ...
  (* ---- Stream ---- *)
  | Parse_stream_start ->
    let tok = get_tok p in  (* STREAM_START *)
  ...
  | Parse_document_start_implicit ->
    ...
    | Some Document_end ->
      if had_dir then
        Types.parse_error sp "directive(s) not followed by a document (unexpected '...')";
  ...
  | Parse_document_content ->
    (match peek_kind p with
    | Some Document_end | Some Stream_end ->
      p.state <- pop_state p;
      let sp = (peek_tok p).tok_start_pos in
  ...
  (* ---- Nodes ---- *)
  | Parse_block_node | Parse_block_node_or_indentless_sequence | Parse_flow_node ->
    ...
    let allow_indentless = (p.state = Parse_block_node_or_indentless_sequence) in
    let in_flow          = (p.state = Parse_flow_node) in
    parse_node p ~allow_indentless ~in_flow

  (* ---- Block sequences ---- *)
  | Parse_block_sequence_first_entry ->
    ignore (get_tok p);  (* BLOCK_SEQUENCE_START *)
  ...
  | Parse_block_sequence_entry ->
    (match peek_kind p with
    | Some Block_entry ->
      let tok = get_tok p in  (* BLOCK_ENTRY *)
      (match peek_kind p with
      | Some Block_entry | Some Block_end ->
        p.state <- Parse_block_sequence_entry;
        (* Empty item *)
    ...
    | Some kind ->
      Types.parse_error (peek_tok p).tok_start_pos
        "expected block sequence entry or BLOCK_END, got %s" (show_kind kind)
    | None ->
      Types.parse_error (peek_tok p).tok_start_pos "unexpected end of tokens in block sequence")
  ...
  | Parse_indentless_sequence_entry ->
    ...
    | Some Block_entry ->
      let tok = get_tok p in
      (match peek_kind p with
      | Some Block_entry | Some Key | Some Value | Some Block_end ->
  ...
  (* ---- Block mappings ---- *)
  | Parse_block_mapping_first_key ->
    ignore (get_tok p);  (* BLOCK_MAPPING_START *)
  ...
  | Parse_block_mapping_key ->
    ...
    | Some Key ->
      let tok = get_tok p in
      (match peek_kind p with
      | Some Key | Some Value | Some Block_end ->
    ...
    | None ->
      Types.parse_error (peek_tok p).tok_start_pos "unexpected end of tokens in block mapping")
  ...
  (* ---- Flow sequences ---- *)
  | Parse_flow_sequence_first_entry ->
    let fs_tok = get_tok p in  (* FLOW_SEQUENCE_START *)
  ...
  | Parse_flow_sequence_entry_mapping_key ->
    (match peek_kind p with
    | Some Value | Some Flow_entry | Some Flow_sequence_end ->
  ...
  | Parse_flow_sequence_entry_mapping_value ->
    ...
    | Some Value ->
      ignore (get_tok p);
      (match peek_kind p with
      | Some Flow_entry | Some Flow_sequence_end ->
  ...
  (* ---- Flow mappings ---- *)
  | Parse_flow_mapping_first_key ->
    ignore (get_tok p);  (* FLOW_MAPPING_START *)
  ...
  | Parse_flow_mapping_key ->
    ...
    | Some Key ->
      let tok = get_tok p in
      (match peek_kind p with
      | Some Value | Some Flow_entry | Some Flow_mapping_end ->
  ...
  | Parse_flow_mapping_value ->
    ...
    | Some Value ->
      ignore (get_tok p);
      (match peek_kind p with
      | Some Flow_entry | Some Flow_mapping_end ->
...
(** Parse a node (scalar, collection, or alias).
    ...
    collection or scalar production. *)
and parse_node p ~allow_indentless ~in_flow:_ =
  let sp = (peek_tok p).tok_start_pos in
  (* Handle alias *)
  if check p [Alias ""] |> fun _ -> (match peek_kind p with Some (Alias _) -> true | _ -> false)
  then begin
    let tok = get_tok p in
    let name = match tok.tok_kind with Alias n -> n | _ -> assert false in
    p.state <- pop_state p;
    mk_event (Alias name) tok.tok_start_pos tok.tok_end_pos
  end else begin
    let (anchor, tag, prop_start) = collect_node_properties p in
    let node_start = match prop_start with Some s -> s | None -> sp in
    (match peek_kind p with
    | Some Block_sequence_start ->
      p.state <- Parse_block_sequence_first_entry;
      let ev_sp = (peek_tok p).tok_start_pos in
      mk_event (Sequence_start
        { anchor; tag; implicit = tag = None; flow = false })
        node_start ev_sp
    ...
    | Some Flow_sequence_start ->
      ...
      mk_event (Sequence_start
        { anchor; tag; implicit = tag = None; flow = true })
        node_start ev_sp
    ...
    | Some (Scalar (value, style)) ->
      let tok = get_tok p in
      p.state <- pop_state p;
      mk_event (Scalar { anchor; tag; value; style })
        tok.tok_start_pos tok.tok_end_pos
    | _ ->
      ...
      | None, None ->
        empty_scalar node_start
      | _ ->
        (* Anchor/tag with no value: empty scalar *)
        mk_event (Scalar { anchor; tag; value = ""; style = Plain })
...
let get_event (p : t) : event =
  match p.events with
  | ev :: rest -> p.events <- rest; ev
  | []         -> produce p

(** Check whether the next event's kind is in the list.
    Uses structural equality on the constructors only (ignores fields). *)
...
let to_event_list (p : t) : event list =
  ...
  while not !stop do
    ...
    result := ev :: !result;
    (match ev.kind with Stream_end -> stop := true | _ -> ())
  done;
  List.rev !result
