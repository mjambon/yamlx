(** Core types shared by all YAMLx modules.
    This module defines the data structures that flow through the parsing
    pipeline: positions and errors, then tokens (Scanner → Parser), events
    (Parser → Composer), and AST nodes (Composer → user).

    Pipeline overview:
      string/channel
        └─▶ Reader   (UTF-8 decoding, line tracking)
        └─▶ Scanner  (tokenisation)    → token list
        └─▶ Parser   (grammar)         → event list
        └─▶ Composer (node building)   → node list
        └─▶ Resolver (type resolution) → value list  *)

(** {1 Source positions} *)

(** A location in the source input.
    [line] is 1-based; [column] is 0-based (Unicode codepoints from line
    start, matching the YAML specification's column numbering). *)
type pos = {
  line   : int;
  column : int;
...
(** How a scalar was written in the YAML source.
    The style is preserved through the parsing pipeline because it affects
    tag resolution and round-trip fidelity. *)
type scalar_style =
...
(** A token represents one lexical element.  The YAML grammar requires the
    ...
    BLOCK_MAPPING_START, BLOCK_END) in addition to the characters that appear
    in the source. *)
type token_kind =
  | Stream_start
  | Stream_end
  ...
  | Block_sequence_start
      (** Synthetic: emitted when a block sequence begins (indentation
          increases to a new level). *)
  | Block_mapping_start
      (** Synthetic: emitted when a block mapping begins. *)
  | Block_end
...
(** Nodes are the in-memory representation of a parsed YAML document.
    Anchors are resolved at compose time so [Alias_node] carries the
    actual target node. *)
type node =
  | Scalar_node of {
      anchor : string option;
...
(** Typed values produced by applying the YAML 1.2 JSON schema to a
    composed node tree. *)
type value =
  | Null
  | Bool   of bool
