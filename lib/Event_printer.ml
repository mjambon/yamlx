(** Convert a parser event list to the yaml-test-suite tree notation.
    ...
    ~~~~~~~~~~~~~~~~
      ...
      =VAL [&anchor] [<tag>] STYLE VALUE
        where STYLE is one of:
          :  plain
          '  single-quoted
          double-quote character for double-quoted
          |  literal block
          >  folded block
      =ALI *name      alias
    ...
    The VALUE in =VAL lines has special characters escaped:
      ...
      actual tab       -> backslash-t
      actual backslash -> double-backslash

    The tree lines in the test suite have leading spaces for visual nesting;
    we strip those when comparing (see [normalise_tree]). *)
...
(** Escape a scalar value string for display in the tree format.
    Newlines, tabs, backslashes, and carriage returns are made visible.
    Trailing spaces are replaced with U+2423 OPEN BOX (␣) per the
    yaml-test-suite convention, so they are not invisible in the output. *)
let escape_value (s : string) : string =
  ...
  let i = ref n in
  while !i > 0 && escaped.[!i - 1] = ' ' do decr i done;
  if !i = n then escaped
  else begin
...
(** Render a single event as a tree-format line.  Returns one string per
    event. *)
let event_to_line (ev : event) : string =
  ...
  | Document_start { explicit; _ } ->
    if explicit then "+DOC ---" else "+DOC"
...
(** Convert an event list to the multi-line tree string.
    Each event becomes one line; lines are separated by newlines.
    The result ends with a final newline. *)
...
(** Split [s] into trimmed, non-empty lines for comparison.
    Leading whitespace (the visual indentation in the test suite's tree
    format) is stripped since it carries no semantic meaning. *)
...
(** Return a human-readable diff between [expected] and [actual] tree
    strings.  Only lists the first differing line. *)
